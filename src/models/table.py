...
def create_table(master_id, type, game, name, max_players, description, image, num_sessions):
...
def update_table(table_id, description, max_players):
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE tables SET description = ?, max_players = ? WHERE id = ?", (description, max_players, table_id))
    conn.commit()
    conn.close()
