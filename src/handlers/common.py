...
def format_table_message(table: Dict[str, Any], master_name: Optional[str] = None, 
                         current_players: int = 0) -> str:
    """
    ...
    # Emoji per diversi tipi di gioco
    game_configs = {
        "zombicide chronicles": {
            "icon": "🧟‍♂️",
            "color_emoji": "🟣",
            "theme": "apocalisse zombie"
        },
        "d&d": {
            "icon": "🐉", 
            "color_emoji": "🔴",
        ...
        "call of cthulhu": {
            "icon": "🐙",
            "color_emoji": "🟢", 
            "theme": "horror cosmico"
        },
        "cyberpunk": {
            "icon": "🤖",
            "color_emoji": "🟡",
            "theme": "distopia futuristica"
        },
        "pathfinder": {
            "icon": "⚔️",
            "color_emoji": "🟠",
            "theme": "fantasy avventuroso"
        }
    }
    ...
    # Header stylizzato
    message = f"╭─── {config['color_emoji']} <b>{config['icon']} {table['game'].upper()}</b> {config['color_emoji']} ───╮\n\n"
    ...
    if table['type'] == 'one_shot':
        ...
        sessions_text = f" ({table['num_sessions']} sessioni)" if 'num_sessions' in table.keys() else ""
    ...
    # Tema del gioco
    message += f"🎨 <b>Ambientazione:</b> {config['theme']}\n"
...
async def tables(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for table in active_tables:
        ...
        # Create buttons
        if is_registered:
            join_button = InlineKeyboardButton("❌ Unjoin", callback_data=f"unjoin_{table['id']}")
        else:
            if current_players >= table['max_players']:
                join_button = InlineKeyboardButton("🚫 Table Full", callback_data=f"table_full_{table['id']}")
            else:
                join_button = InlineKeyboardButton("✅ Join", callback_data=f"join_{table['id']}")

        keyboard = [
            [join_button,
             InlineKeyboardButton("👥 Show Players", callback_data=f"show_{table['id']}")],
        ]
...
async def join_table_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
async def unjoin_table_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
async def show_players_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not registrations:
        ...
        for i, player in enumerate(registrations, 1):
            username = f"@{player['username']}" if 'username' in player.keys() else "no username"
            message += f"   {i}. {player['first_name']} {player['last_name']} ({username})\n"
